...
export default {
  data () {
    return {
      url: 'http://test.teachingtoys.co.th' + this.$route.path + '/',
      item: { name: 'testname' },
      item2: null
    }
  },
  ...
  asyncData ({ route }) {
    ...
    // var _self = this
    return axios.get('http://localhost:8000/api/products/' + route.params.id)
      .then((res) => {
  ...
  methods: {
    getData (item) {
      var _self = this
      axios.get('http://localhost:8000/api/products/' + item)
        .then((res) => {
